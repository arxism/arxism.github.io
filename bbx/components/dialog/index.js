...
const setupTabs = (dialog) => {
    ...
    const tabs = dialog.querySelectorAll(eid.tab);
    tabs.forEach(tab => {
        tab.addEventListener('click', onTabClick);
    });
    const [left, right] = Array.from(dialog.querySelectorAll(eid.tabs));
    left?.querySelector('[data-position="left"]')?.click();
...
export const render = async (props) => {
    ...
    const panes = props.panes.reduce((panes, pane) => {
        ...
        if (pane.content)
            content.innerHTML = pane.content;
        if (pane.onBlur)
            content.addEventListener('blur', pane.onBlur);
        ...
        if (panels instanceof HTMLDivElement)
